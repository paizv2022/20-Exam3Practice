...
def run_test_integers():
    ...
    #Test 2:
    expected = [1, 2, 3, 4, 5]
    actual = integers([(1, 1.5, 2, 3.7, 4.6), [3, 8.9999, 4, 6.87, 5]])
    print('Expected is:', expected)
    ...
    for j in range(len(sequence_of_sequences)):
        for k in range(len(sequence_of_sequences[j])):
            if type(sequence_of_sequences[j][k]) is int:
                list_nums.append(sequence_of_sequences[j][k])
    return list_nums
...
def run_test_big_letters():
    ...
    expected = 'HELLO'
    actual = big_letters(['Hello and', 'goodbyE', 'hjkLL', (5, 6, 7, 'O'), 'jellO'])
    print('Expected is:', expected)
    print('Actual is:  ', actual)
            ...
            if type(sequence_of_sequences[j]) is str:
                if type(sequence_of_sequences[j][k]) is str:
                    if sequence_of_sequences[j][k].isupper():
                        sting_upper = sting_upper + sequence_of_sequences[j][k]
    return sting_upper
